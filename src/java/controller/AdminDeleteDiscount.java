...
public class AdminDeleteDiscount extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try
        {
            ...
            boolean discountDelete = _discountRepository.DeleteDiscount(discountid);
            if(discountDelete)
            {
                session.removeAttribute("discounts");
                message = DISCOUNT_DELETE_SUCCESS.getMessage();
            }
        }
        catch(Exception ex)
        {
            message = DISCOUNT_DELETE_FAIL.getMessage();
            System.out.println("delete discount failed " + ex.getMessage());
        }
