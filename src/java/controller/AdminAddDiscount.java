...
public class AdminAddDiscount extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String message = DISCOUNT_ADD_FAIL.getMessage();
        try
        {
            ...
            Discount discount = new Discount(name, code, percent);
                    
            boolean discounts = _discountRepository.UpSertDiscount(discount);
            if(discounts)
            {
                session.removeAttribute("discounts");
                message = DISCOUNT_ADD_SUCCESS.getMessage();
            }
        }
        catch(Exception ex)
        {
            System.out.println("get discount failed " + ex.getMessage());
        }
